...
import wallet.core.jni.CoinType.SAFECOIN
import wallet.core.jni.proto.Safecoin
import wallet.core.jni.proto.Safecoin.SigningOutput

class TestSafecoinSigner {
    ...
    fun testTransferSign() {
        val transferMessage = Safecoin.Transfer.newBuilder().apply {
            recipient = "EN2sCsJ1WDV8UFqsiTXHcUPUxQ4juE71eCknHYYMifkd"
            value = 42
        }.build()
        val signingInput = Safecoin.SigningInput.newBuilder().apply {
        ...
        val output = AnySigner.sign(signingInput, SAFECOIN, SigningOutput.parser())
    ...
    fun testDelegateStakeSign() {
        val delegateStakeMessage = Safecoin.Stake.newBuilder().apply {
    ...
    fun testDeactivateStakeSign() {
        val deactivateStakeMessage = Safecoin.DeactivateStake.newBuilder().apply {
            validatorPubkey = commonValidatorPubkey
        }.build()
        val signingInput = Safecoin.SigningInput.newBuilder().apply {
    ...
    fun testWithdrawStakeSign() {
        val withdrawStakeMessage = Safecoin.WithdrawStake.newBuilder().apply {
            validatorPubkey = commonValidatorPubkey
            value = 42
        }.build()
        val signingInput = Safecoin.SigningInput.newBuilder().apply {
    ...
    fun testCreateTokenAccountSign() {
        val createAccountMessage = Safecoin.CreateTokenAccount.newBuilder().apply {
    ...
    fun testTokenTransferSign() {
        val tokenTransferMessage = Safecoin.TokenTransfer.newBuilder().apply {
            tokenMintAddress = "SRMuApVNdxXokk5GT7XD5cUUgXMBCoAz2LHeuAoKWRt"
    ...
    fun testCreateAndTransferTokenSign() {
        val createAndTransferTokenMessage = Safecoin.CreateAndTransferToken.newBuilder().apply {
        ...
        //https://explorer.safecoin.org/tx/449VaYo48LrkMJF6XVKt9sJwVQN6Seqrmh9erDCLtiuj6BgFG3wpF5TwjNkxgJ7qzNa6NTj3TFsU3h9hKszfkA7w
